...
/*    { 
    ...
    double w1;
    {
        w1 = log(VectorField(4)+1)*1.0+0.001;       
    }
...
/*    if( ret )
    ...
    poseStamped.pose.position.x = target_ori[0];
    poseStamped.pose.position.y = target_ori[1];
    poseStamped.pose.position.z = 0.0;

    geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(target_ori[2]);
...
//    return VectorXd::Zero(2);
    ...
    else
    {
        ...
        target_->setYaw(target_d[2]+randValue);
        return VectorXd::Zero(2);
    }
    return VectorXd::Zero(2);
...
//    return VectorXd::Zero(2);
    ...
    else
    {
        ...
        double randValue = double(rand()%(20*100))/100.0*3.1415/180.0;
        int randSign = rand()%2;
        if( randSign != 1 )
        {
            randValue*=-1.0;
...
{
    ros::init(argc, argv, "heightmap_node");  //?몃뱶�??�덇�??    

    ...
    for (int x = 0; x < info.width; x++)
    {
        for (int y = 0; y < info.height; y++)
        {
            ...
            /*double dx = -grid_offset_x + (x*m_per_cell_+m_per_cell_/2.0);
                ...
                cout <<"inserted "<<obsval<<" "<<x<<" " <<y <<" "<<info.width <<" "<<info.height<<endl;
                QuadTree::Cell(x,y,info.width,obsval);
                cout <<"inserting.."<<endl;
                */
            obsval = 0.99;    
            map.Insert(QuadTree::Cell(x,y,info.width,obsval));
            cout <<"inserted end"<<endl;
        }
    }
...
bool DO( double* from_d, double* to_d, double* target_d)
{
    /*
    {
        ...
        poseStamped.pose.position.x = from_d[0];
        poseStamped.pose.position.y = from_d[1];
        poseStamped.pose.position.z = 0.0;

        geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(from_d[2]);
...
//    return VectorXd::Zero(2);
    ...
    else
    {
        ...
        VectorXd VectorField = ComputePotentialField2(target_d[0],target_d[1],target_d[2],START,GOAL);
...
//    return VectorXd::Zero(2);
    ...
    else
    {
        ...
        cout << "* "<<target_->getYaw() <<endl;
        target_->setYaw(target_d[2]);

        ob::SE2StateSpace::StateType *target__ = target->as<ob::SE2StateSpace::StateType>();
        cout << "- "<<target__->getYaw() <<endl;
...
void plan(ob::StateSpacePtr space,double time, bool isClothoid, bool isRRT)
{
    ...
    for(int i=0; i<ITER; i++)
    {
        ...
        if( isRRT )
        {
            if( isClothoid )
            {
                ss.setPlanner(std::make_shared<ompl::geometric::DORRT>(ss.getSpaceInformation(),randomCheck,magneticfield, DesiredOrientation));
            }
            else
            {
                ss.setPlanner(std::make_shared<ompl::geometric::DORRT>(ss.getSpaceInformation(),randomCheck_,magneticfield_, DesiredOrientation_));
            }
        }
        else
        {
...
int main(int argc, char* argv[])
{
    ...
    potential_publisher_= node.advertise<geometry_msgs::PoseArray>("RANDOMVEC", 1000);
    potential3_publisher_= node.advertise<geometry_msgs::PoseArray>("RANDOMVEC2", 1000);
    potential2_publisher_= node.advertise<geometry_msgs::PoseArray>("POTENTIAL", 1000);
    map_pub= node.advertise<nav_msgs::OccupancyGrid>("GRIDMAP", 1000);
    msgpub3 = node.advertise<nav_msgs::Path>("RESULTPATH", 1000);
    try
    {

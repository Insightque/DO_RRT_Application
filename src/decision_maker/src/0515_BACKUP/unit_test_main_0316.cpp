...
//    if( start_dist < 5.0 )
    ...
    ret(4) = (sqrt(ret(2)*ret(2)+ret(3)*ret(3)) - BMin)*BRange;
    ret(4) *= 10.0;
    {
    //    ret(2) = ret(2)/sqrt(ret(2)*ret(2)+ret(3)*ret(3));
    //    ret(3) = ret(3)/sqrt(ret(2)*ret(2)+ret(3)*ret(3));
    }
...
//    double w1= log(VectorField(4)+1)*0.5+0.001;       
    ...
    else
    {
        ...
        double BValue = (sqrt(B[0]*B[0]+B[1]*B[1]) - BMin)*BRange;
        ...
        {
            vGradient(0) = cos(target_d[2]+RADIANS(180));
            vGradient(1) = sin(target_d[2]+RADIANS(180));
            w1 = 0.3;
...
bool DesiredOrientation__(ob::State* from, ob::State* to, ob::State* target)
{
    return true;
}
...
//    return VectorXd::Zero(2);
    ...
    else
    {
        ...
        double randValue = -1*(VectorField(4)-0.4)/(0.3+abs(-(VectorField(4)-0.4)))+0.95;
        if( randValue < 0 )
            randValue = 0;
        if( randValue > 1 )
            randValue = 1;
        ...
        double randAng =0.0;
        if( randValue >=1.0 )
            randAng = (rand()%int(randValue))/100.0;
        ...
        randValue=0.0;
        //cout << randAng*180/3.141592 << endl;
        target_d[2] =AngleUtils::toRange_PItoPI( atan2(vGradient(1), vGradient(0))+randAng);
        target_->setYaw(target_d[2]);
        return VectorXd::Zero(2);
    }
    return VectorXd::Zero(2);
...
/*    if( isClothoid )
    ...
    */
    ...
    else
    {
        ...
        //b.addPlanner(std::make_shared<ompl::geometric::DORRTstar>(ss.getSpaceInformation(),randomCheck_,magneticfield_, DesiredOrientation__));
    }

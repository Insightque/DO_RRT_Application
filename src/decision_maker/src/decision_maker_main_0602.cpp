...
void setEnvironment(og::SimpleSetup* ss, int type)
{
...
/*
  ...
  */ 
    ...
    bool bBefore=isValid(x, y, yaw);
    bool bAfter=isValid(x, y, yaw_);
    ...
    cout<<x<<"\t"<<y<<"\t"<<yaw<<"\t"<<yaw_<<"\t"<<nBefore<<"\t"<<nAfter<<endl;
}
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        else if( vm.count("conf"))
        {
            ...
            for(double x=10.0; x<30.0; x+=2.0)
                for(double y=25.0; y<45.0; y+=2.0)
                    for(double yaw=RADIANS(-180.0); yaw<=RADIANS(180.0); yaw+=RADIANS(2.0))

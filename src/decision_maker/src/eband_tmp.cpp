...
bool EbandManager::ComputeNewtonRaphton()
{
    ...
    while(iter <1000)
    {
        ...
        for( int i=1; i<mNodes_local.size()-1 ; i++)
        {
        //    double li = ((mNodes_local1.at(i) - mNodes_local1.at(i-1)).norm()+(mNodes_local1.at(i) - mNodes_local1.at(i+1)).norm())/2.0;
            k = i-1;
            ...
            // internal springs
            ComputeIntForce(mNodes_local.at(i),mNodes_local.at(i+1),mNodes_local.at(i-1),ki_,li,F_int);
            ComputeIntJacobian(mNodes_local.at(i),mNodes_local.at(i+1),mNodes_local.at(i-1),ki_,li,J_int,_J_int,J_int_);
            ...
            Vector2d vec1 =  Vector2d(mNodes_local1.at(i)(0),mNodes_local1.at(i)(1))+Rot*Vector2d(1,0); // Cm_ --> (2,1)
            ...
            Vector2d vec2 =  Vector2d(mNodes_local1.at(i)(0),mNodes_local1.at(i)(1))+Rot*Vector2d(1,0); // Cm_ --> (2,1)
            //Vector2d vec2 =  Vector2d(mNodes_local1.at(i)(0),mNodes_local1.at(i)(1)); // Cm_ --> (2,1)


            ComputeIntForce(mNodes_local.at(i),vec1,vec2,ki,1.0,F1_int);
            ComputeIntJacobian(mNodes_local.at(i),vec1,vec2,ki,1.0,J1_int,_J1_int,J1_int_);
            ...
            for( int j=0; j<obs.size(); j++)
            {
                ...
                if( (vObs - mNodes_local.at(i)).norm() == 0 )
                {
                    Vector2d tmpVec=Vector2d(mNodes_local1.at(i)(0),mNodes_local1.at(i)(1));
                    vObs += (tmpVec-mNodes_local.at(i))/ (tmpVec-mNodes_local.at(i)).norm();
...
//            cout << J_obs << endl;
            if( k > 0 )
            {
                mJ(k*2+0,k*2-2) = _J_int(0,0)+_J1_int(0,0);
                mJ(k*2+0,k*2-1) = _J_int(0,1)+_J1_int(0,1);
                mJ(k*2+1,k*2-2) = _J_int(1,0)+_J1_int(1,0);
                mJ(k*2+1,k*2-1) = _J_int(1,1)+_J1_int(1,1);
            }
            ...
            {
                mJ(k*2+0,k*2+2) = J_int_(0,0)+J1_int_(0,0);
                mJ(k*2+0,k*2+3) = J_int_(0,1)+J1_int_(0,1);
                mJ(k*2+1,k*2+2) = J_int_(1,0)+J1_int_(1,0);
                mJ(k*2+1,k*2+3) = J_int_(1,1)+J1_int_(1,1);
            }
        ...
        double epsilon = mm.norm();
        cout <<"F, J computed: "<< epsilon<<endl;
        if( epsilon < tEpsilon )
        {
            ret = true;
            break;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Internal Springs
void EbandManager::ComputeIntForce(Vector2d node, Vector2d node_, Vector2d _node, double k, double l_init,MatrixXd& F)
{
    double ri_x=        node.x();
    double ri_y=        node.y();
    ...
    F(0,0) = ki*(ri_x*2.0-rim1_x*2.0)*1.0/sqrt(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0))*(li-sqrt(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0)))*(1.0/2.0)+ki*(ri_x*2.0-rip1_x*2.0)*1.0/sqrt(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0))*(li-sqrt(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0)))*(1.0/2.0);
    F(1,0) = ki*(ri_y*2.0-rim1_y*2.0)*1.0/sqrt(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0))*(li-sqrt(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0)))*(1.0/2.0)+ki*(ri_y*2.0-rip1_y*2.0)*1.0/sqrt(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0))*(li-sqrt(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0)))*(1.0/2.0);
...
void EbandManager::ComputeIntJacobian(Vector2d node, Vector2d node_, Vector2d _node, double k, double l_init,MatrixXd& J,MatrixXd& _J,MatrixXd& J_)
{
    double ri_x=        node.x();
    double ri_y=        node.y();
    double rip1_y=      node_.y();
    ...
    J(0,0) = ki*-2.0+ki*li*(ri_y*ri_y)*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)+ki*li*(ri_y*ri_y)*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)+ki*li*(rim1_y*rim1_y)*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)+ki*li*(rip1_y*rip1_y)*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)-ki*li*ri_y*rim1_y*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)*2.0-ki*li*ri_y*rip1_y*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)*2.0;
    J(0,1) = (ki*(ri_x*2.0-rim1_x*2.0)*(ri_y*2.0-rim1_y*2.0)*(-1.0/4.0))/(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0))-(ki*(ri_x*2.0-rip1_x*2.0)*(ri_y*2.0-rip1_y*2.0)*(1.0/4.0))/(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0))-ki*(ri_x*2.0-rim1_x*2.0)*(ri_y*2.0-rim1_y*2.0)*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)*(li-sqrt(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0)))*(1.0/4.0)-ki*(ri_x*2.0-rip1_x*2.0)*(ri_y*2.0-rip1_y*2.0)*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)*(li-sqrt(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0)))*(1.0/4.0);
    J(1,0) = (ki*(ri_x*2.0-rim1_x*2.0)*(ri_y*2.0-rim1_y*2.0)*(-1.0/4.0))/(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0))-(ki*(ri_x*2.0-rip1_x*2.0)*(ri_y*2.0-rip1_y*2.0)*(1.0/4.0))/(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0))-ki*(ri_x*2.0-rim1_x*2.0)*(ri_y*2.0-rim1_y*2.0)*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)*(li-sqrt(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0)))*(1.0/4.0)-ki*(ri_x*2.0-rip1_x*2.0)*(ri_y*2.0-rip1_y*2.0)*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)*(li-sqrt(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0)))*(1.0/4.0);
    J(1,1) = ki*-2.0+ki*li*(ri_x*ri_x)*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)+ki*li*(ri_x*ri_x)*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)+ki*li*(rim1_x*rim1_x)*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)+ki*li*(rip1_x*rip1_x)*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)-ki*li*ri_x*rim1_x*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)*2.0-ki*li*ri_x*rip1_x*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)*2.0;


    _J(0,0) = -ki*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)*(li*(ri_y*ri_y)+li*(rim1_y*rim1_y)-pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)-li*ri_y*rim1_y*2.0);
    _J(0,1) = ki*li*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)*(ri_x-rim1_x)*(ri_y-rim1_y);
    _J(1,0) = ki*li*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)*(ri_x-rim1_x)*(ri_y-rim1_y);
    _J(1,1) = -ki*1.0/pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)*(li*(ri_x*ri_x)+li*(rim1_x*rim1_x)-pow(pow(ri_x-rim1_x,2.0)+pow(ri_y-rim1_y,2.0),3.0/2.0)-li*ri_x*rim1_x*2.0);


    J_(0,0) = -ki*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)*(li*(ri_y*ri_y)+li*(rip1_y*rip1_y)-pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)-li*ri_y*rip1_y*2.0);
    J_(0,1) = ki*li*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)*(ri_x-rip1_x)*(ri_y-rip1_y);
    J_(1,0) = ki*li*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)*(ri_x-rip1_x)*(ri_y-rip1_y);
    J_(1,1) = -ki*1.0/pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)*(li*(ri_x*ri_x)+li*(rip1_x*rip1_x)-pow(pow(ri_x-rip1_x,2.0)+pow(ri_y-rip1_y,2.0),3.0/2.0)-li*ri_x*rip1_x*2.0);
...
void EbandManager::ComputeObsForce(Vector2d node, Vector2d obstacle, double k, double r, MatrixXd &F)
{
    ...
    {
        F(0,0) += -kobs*1.0/sqrt(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0))*(r_obs_x-ri_x)*(r_safe-sqrt(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0)));
        F(1,0) += -kobs*1.0/sqrt(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0))*(r_obs_y-ri_y)*(r_safe-sqrt(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0)));
    }
    else
    {
        F(0,0) += 0.0;
 ...
 */
    ...
    J(0,1) += -(kobs*signKx*fabs(r_obs_x-ri_x)*(r_obs_y-ri_y))/(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0))-kobs*signKx*fabs(r_obs_x-ri_x)*1.0/pow(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0),3.0/2.0)*(r_obs_y-ri_y)*(r_safe-sqrt(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0)));
    J(1,0) += -(kobs*signKy*fabs(r_obs_y-ri_y)*(r_obs_x-ri_x))/(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0))-kobs*signKy*fabs(r_obs_y-ri_y)*1.0/pow(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0),3.0/2.0)*(r_obs_x-ri_x)*(r_safe-sqrt(pow(fabs(r_obs_x-ri_x),2.0)+pow(fabs(r_obs_y-ri_y),2.0)));

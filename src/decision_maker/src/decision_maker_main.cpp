...
void publish_path1(oc::SimpleSetup* ss)
{
    std_msgs::Header header;
    header.stamp = ros::Time::now();
#ifdef DRAWVEHICLE
...
public:
    ...
    KinematicCarModel(const ob::StateSpace *space) : space_(space), carLength_(3.8)
    {
    }
...
public:
    ...
    DemoStatePropagator(const oc::SpaceInformationPtr &si) : oc::StatePropagator(si),
                                                             integrator_(si->getStateSpace().get(), 0.01)
    {
    }
...
void plan_control(double time, double* pStart, double* pGoal, int type)
{
...
//    g_msgsub1 = node.subscribe("LocalizationData", 1, &SubTopicProcess1, this);
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    //ob::StateSpacePtr space(new ob::DubinsStateSpace(5.88, true)); // false: forward
    ...
    double clow_wheel = -RADIANS(25.0);
    double chigh_wheel = RADIANS(25.0);

    double clow = 0.0*0.278;
    double chigh = 3.0*0.278;
    ...
    ompl::base::ScopedState<ob::SE2StateSpace> start(space);
    start->setX(pStart[0]);
    start->setY(pStart[1]);
    start->setYaw(pStart[2]);

    ompl::base::ScopedState<ob::SE2StateSpace> goal(space);
    goal->setX(pGoal[0]);
    goal->setY(pGoal[1]);
    goal->setYaw(pGoal[2]);
    ...
    {
        ompl::base::PlannerPtr planner(new ompl::control::DORRT(ss.getSpaceInformation(),randomCheck,magneticfield, DesiredOrientation));
        ss.setPlanner(planner);
    }
...
void benchmark_control(double time, double* pStart, double* pGoal)
{
    ...
    // set the start and goal states
    ss.setStartAndGoalStates(start, goal,2.5);
    ...
    //b.addPlanner(ompl::base::PlannerPtr(new ompl::control::DORRT(ss.getSpaceInformation(), randomCheck_, magneticfield,DesiredOrientation)));
...
void planner_eband(og::SimpleSetup* ss)
{
    ...
    for(int i=0;i<path.getStateCount(); i++)
    {
        ...
        if( i>=tar_goalIndex && goalIndex==-1 )
        {
    ...
    cout <<BOUNDS.low[0]<<" "<<BOUNDS.high[0]<<" "<<BOUNDS.low[1]<<" "<<BOUNDS.high[1]<<endl;

    setTarget(ss, start, goal);
    cout <<start[0]<< " "<<start[1] <<" "<<goal[0]<<" "<<goal[1]<<endl;
    plan(ss,0.2);
    ...
    for(int i=0;i<path.getStateCount(); i++)
    {
        ...
        {
            vpath.push_back(Vector2d(x,y));
            vpath1.push_back(Vector3d(x,y,yaw));
        }*/
    }
    /*
    cout<<"-------------------" <<endl;
    EbandManager tmp;
    tmp.SetLocalNodes(vpath,vpath1, OBSTACLE);
    //tmp.SetLocalNodes();
    cout <<" compute Newton"<<endl;
    if( tmp.ComputeNewtonRaphton() )
    {

    }
      for( int i=0; i<tmp.mNodes_local.size();i++)
      {
          cout<<tmp.mNodes_local.at(i)(0)<<"\t"<<tmp.mNodes_local.at(i)(1)<<"   "<<tmp.mNodes_local1.at(i)(0)<<"\t"<<tmp.mNodes_local1.at(i)(1)<<endl;
      }
      */
}
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        else if( vm.count("control"))
        {
            setEnvironment(env,start,goal);
            cout << start[0] << "\t" << start[1] <<"\t"<<start[2]<<"\t"<<goal[0]<<"\t"<<goal[1]<<"\t"<<goal[2]<<endl;
            plan_control(PLANNINGTIME, start, goal,type);
            //benchmark_control(PLANNINGTIME, start, goal);

    ...
    switch(type)
    {
        ...
        case 1:
            ...
            for(double x=8; x<10; x+=0.2 )
                for(double y=2; y<15; y+=0.2)
                    OBSTACLE.push_back(Vector2d(x,y));
            for(double x=25; x<27; x+=0.2 )
                for(double y=-15; y<5; y+=0.2)
                    OBSTACLE.push_back(Vector2d(x,y));
            for(double x=65; x<67; x+=0.2 )
                for(double y=-13; y<5; y+=0.2)
                    OBSTACLE.push_back(Vector2d(x,y));
            for(double x=50; x<65; x+=0.2 )
                for(double y=0; y<2; y+=0.2)
                    OBSTACLE.push_back(Vector2d(x,y));
            for(double x=-12; x<80; x+=0.2 )
...
bool isFreeSpace(const vector<vector<VectorXd> >& VectorObstacleMap,int i, int j)
{
...
bool isValid(const vector<vector<VectorXd> >& VectorObstacleMap,double x, double y, double yaw)
{
    double x_ = x;
    double y_ = y;
    ...
    bool isFreeSpace1 = isFreeSpace(VectorObstacleMap, i, j);
    ...
    bool isFreeSpace2 = isFreeSpace(VectorObstacleMap, i, j);
    ...
    bool isFreeSpace3 = isFreeSpace(VectorObstacleMap, i, j);
    ...
    bool isFreeSpace4 = isFreeSpace(VectorObstacleMap, i, j);
    ...
    bool isFreeSpace5 = isFreeSpace(VectorObstacleMap, i, j);
    
    i= int((vehicle_back_x +grid_offset_x - m_per_cell_/2.0 )/m_per_cell_);
    j= int((vehicle_back_y +grid_offset_y - m_per_cell_/2.0 )/m_per_cell_);
    
    bool isFreeSpace6 = isFreeSpace(VectorObstacleMap, i, j);
    //////////////////////////////////////////////////////////////////////////////
    x = x_ + cos(yaw-RADIANS(90.0));
    y = y_ + sin(yaw-RADIANS(90.0));
    ...
    bool isFreeSpace7 = isFreeSpace(VectorObstacleMap, i, j);
    ...
    bool isFreeSpace8 = isFreeSpace(VectorObstacleMap, i, j);
    
    i= int((vehicle_back_x +grid_offset_x - m_per_cell_/2.0 )/m_per_cell_);
    j= int((vehicle_back_y +grid_offset_y - m_per_cell_/2.0 )/m_per_cell_);
    
    bool isFreeSpace9 = isFreeSpace(VectorObstacleMap, i, j);
...
bool randomCheck_(ob::State* rand)
{
    ...
    if( isOriginal )
       return false;
        ...
        return !isFreeSpace(VectorObstacleMap, i, j);
...
bool isStateValid(const ob::SpaceInformation *si, const vector<vector<VectorXd> >& VectorObstacleMap, const ob::State *state)
{
    const ob::SE2StateSpace::StateType *s = state->as<ob::SE2StateSpace::StateType>();
    return si->satisfiesBounds(s) && isValid(VectorObstacleMap, s->getX(),s->getY(),s->getYaw());
}

bool isStateValid_Clothoid(const ob::SpaceInformation *si, const vector<vector<VectorXd> >& VectorObstacleMap, const ob::State *state)
{

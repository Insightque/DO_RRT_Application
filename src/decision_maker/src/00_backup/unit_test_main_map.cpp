...
void VectorFieldGenerator(double* start, double* goal, ob::RealVectorBounds bounds)
{
    ...
    for (int i = 0; i<grid_dim_x; i++)
    {
        for (int j = 0; j<grid_dim_y; j++)
        {
            ...
            VectorXd potVec = ComputePotentialField(x, y, start, goal);
            VectorXd obsVec = ComputeObstacleField(x, y);
            VectorMap[i][j] = potVec;
            VectorObstacleMap[i][j] = obsVec;
...
bool DO( double* from_d, double* to_d, double* target_d)
{
    ...
    if( i >= 0 && i < grid_dim_x_ && j >= 0 && j < grid_dim_y_ )
    {
        ...
        double w1= log(VectorMap[i][j](4)+1)*1.0+0.001;       

        if( VectorMap[i][j](4) > 0 )
        {
...
//    return VectorXd::Zero(2);
    ...
    else
    {
        ...
        double randValue = double(rand()%(45*100))/100.0*3.1415/180.0;
        int randSign = rand()%2;
        if( randSign != 1 )
        {
            randValue*=-1.0;
